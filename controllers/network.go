...
func getNetworks(w http.ResponseWriter, r *http.Request) {
	...
	// --- 1. filtro de “master” (já existente) ---
	if r.Header.Get("ismaster") != "yes" {
		...
		// --- 2. filtro por ROLE (já existente) ---
		allnetworks = logic.FilterNetworksByRole(allnetworks, *user)

		// --- 3. NOVO: filtro por ACL (rede autorizada) ---
		//    Administrador continua vendo tudo; demais usuários só ACL
		if !user.IsAdmin {
			netsAllowed, err := logic.NetworksAllowedToUser(username)
			if err != nil {
				logic.ReturnErrorResponse(w, r, logic.FormatError(err, "internal"))
				return
			}
			// se não houver rede autorizada, deixa slice vazio
			if len(netsAllowed) == 0 {
				allnetworks = []models.Network{}
			} else {
				// filtra só as redes cujo NetID está na lista
				var filtered []models.Network
				for _, net := range allnetworks {
					if sliceContains(netsAllowed, net.NetID) {
						filtered = append(filtered, net)
					}
				}
				allnetworks = filtered
			}
		}
	}
